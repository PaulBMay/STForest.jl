...
function NNGP_ZIST_yMAP(data::InputData, initthetay::Vector, spriors::SpatialPriors, sp::SpatialParams; sub = 1.0, f_tol = 1e-3, g_tol = 1e-3, alpha = 1e-6, show_trace = true, store_trace = false)
    ...
    datap = InputData(data.y[zf], data.X[zf,:], data.loc[zf,:], data.time[zf,:])

    # We'll need a projection to unique locations for the positive AGBD plots, too

    map2unqp = @views indexin(loc2str(datap.loc), loc2str(locUnq[z,:]))

    ####################
    ####################

    nby = getNeighbors(datap.loc, sp.m)

    swy1, rangeSy1, rangeTy1, swy2, rangeTy2, t2y = exp.(initthetay)

    By,Fy,ByOrder = getNNGPmatsST(nby, datap.loc, datap.time, rangeSy1, rangeTy1)

    BtRows, BtCols, BtOrder = getBtNZ(np, map2unqp, nKnots)
    BtCompact = expCor(datap.time, sp.timeKnots, rangeTy2)
    Bt = sparse(BtRows, BtCols, view(vec(BtCompact'), BtOrder))
    Qt =expCor(sp.timeKnots, rangeTy2)

    betayprec = fill(0.01, p)

    Dsgny = sparse_hcat(datap.X, (swy2^2)*Bt, speye(np))
    ...
    QyPostChol = cholesky(Hermitian(QyPost))

    
    #thetayNLP(wutX, spriors, datap, timeKnots, betayprec, nby, By, Fy, ByOrder, Bt, BtCompact, BtOrder, Qt, QyPostChol)
    
    thetayMin = optimize(ty -> thetayNLP(ty, spriors, datap, timeKnots, betayprec, nby, By, Fy, ByOrder, Bt, BtCompact, BtOrder, Qt, QyPostChol), 
                            initthetay, BFGS(alphaguess = Optim.LineSearches.InitialStatic(alpha=alpha)), 
