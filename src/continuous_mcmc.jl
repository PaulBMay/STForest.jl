function NNGP_Continuous_MCMC(data::InputData, initparams::NamedTuple, spriors::NamedTuple, sp::NamedTuple, thetaVar::Matrix, outDir::String, nSamps::Int64; writew2 = false)
    ...
    CSV.write(loctimepOut, DataFrame(lon = data.loc[:,1], lat = data.loc[:,2], time = data.time[:,1]))
    ...
    paramsDf = DataFrame(zeros(1, p + 6), 
        ["betay_".*string.(0:(p-1)); ["sw1", "rangeS1", "rangeT1", "sw2", "rangeT2", "tSq"]]
        )

    w1Out = joinpath(outDir, "wy1.csv")
    w1Df = DataFrame(zeros(1,n), "w1_".*string.(1:np))

    if writew2
        w2Out = joinpath(outDir, "w2.csv")
        w2Df = DataFrame(zeros(1, kt), "w2_l" .* string.(repeat(1:npUnq, inner = nKnots)) .* "_k" .* string.(repeat(1:nKnots, nUnq)) )
    end
    ...
    paramsDf[1,:] = [betay; [sw1, rangeS1, rangeT1, sw2, rangeT2, tSq]]
    w1Df[1,:] = w1
    ...
    B2Compact = expCor(time, sp.timeKnots, rangeT2)
    ...
    for i = ProgressBar(1:nSamps)
       ...
       sw1p, rangeS1p, rangeT1p, sw2p, rangeT2p, tSq = exp.(propTheta)
       ...
       expCor!(Q2, sp.timeKnots, rangeT2p)
       expCor!(B2Compact, data.time, sp.timeKnots, rangeT2p)
