...
function NNGP_Bernoulli2(data::InputData, m::Int64, initparams::NamedTuple, priors::NamedTuple, outDir::String, nSamps::Int64; adaptStart = 50, thetalog = false)
    ...
    thetamat = zeros(nSamps + 1, 5)
    thetamat[1,:] = log.([sw1, rangeS1, rangeT1, sw2, rangeS2])

    thetaVar = 1e-5*Matrix(I,5,5)
    ...
    for i = ProgressBar(1:nSamps)
       ...
       if i >= adaptStart
        thetaVar .= (2.4^2/5)*cov(thetamat[1:i,:])
       end

       currentTheta = thetamat[i,:]
       propTheta = currentTheta + cholesky(thetaVar).L*randn(5)
       ...
       if acceptTheta
            sw1, rangeS1, rangeT1, sw2, rangeS2 = sw1p, rangeS1p, rangeT1p, sw2p, rangeS2p
            thetamat[i+1,:] = propTheta
            B1.nzval .= copy(B1p.nzval)
            F1 .= copy(F1p)
            B2.nzval .= copy(B2p.nzval)
            F2 .= copy(F2p)
       else
            thetamat[i+1,:] = currentTheta
       end
